...
int main(void){
    ...
    err = colirOne.rf.setTxRxAdress((uint8_t*)TxAddress, (uint8_t*)RxAddress);
    if(err != COLIRONE_OK){
        printf("Error setting TX/RX address: %d\n", err);
        return -1;
    }
    err = colirOne.rf.setTxMode();
    if(err != COLIRONE_OK){
	...
	while(1){
        ...
        if(!colirOne.rf.isRxMode()){
            ...
            if(colirOne.rf.hasReceivedData()){
                ...
                printf("Received Command: Launch Number: %d, Open Shutes: %d, Start Logs: %d, Write Logs: %d, Reset Altitude: %d, Remove Logs: %d\n",
                       launchNumber, openShutes, startLogs, writeLogs, resetAltitude, removeLogs);
            }
